// Point d'entrée de l'application Vue.js & Composant racine

// Attendre que le DOM et Vue.js soient chargés
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Charger le composant ArticleList dynamiquement
    // Pour l'instant, on définit ArticleList directement ici
    // (Dans un vrai projet, on utiliserait un bundler ou on chargerait les modules différemment)
    
    // Définir le composant ArticleList (copie de Component/ArticleList.js)
    const ArticleListComponent = {
        name: "ArticleList",
        
        template: `
            <main class="article-list" v-show="page == 'home'">
                <h1>{{ title }}</h1>
                ...
                <article v-if="selectedArticle" class="article-detail">
                    ...
                    <p><img alt="illustration" :src="media_path" /></p>
                    <p>{{ selectedArticle.body }}</p>
                    <button @click="hideArticle(selectedArticle)" class="close-btn">Fermer</button>
                </article>
                ...
                <section class="articles">
                    ...
                    >
                        <h2>{{ article.title }}</h2>
                        <p class="article-resume">{{ article.resume || article.body }}</p>
                        <p v-if="showMore" class="article-content">{{ article.content || article.more }}</p>
    ...
    const App = {
        ...
        data() {
            return {
                currentPage: 'home', // Pour la navigation entre les pages
                menuItems: [
                    { name: 'Accueil', link: 'index.html', page: 'home' },
                    { name: 'Articles', link: 'article.html', page: 'articles' },
                    { name: 'Personnalisation', link: 'custom.html', page: 'custom' }
                ]
            };
        },
        ...
        template: `
            <div id="app">
                <!-- Menu de navigation -->
                <nav class="vue-nav" v-if="currentPage === 'home'">
                    <ul class="vue-nav-list">
                        <li v-for="item in menuItems" :key="item.page" class="vue-nav-item">
                            <a 
                                :href="item.link" 
                                :class="{ 'vue-nav-link': true, 'active': isActivePage(item.page) }"
